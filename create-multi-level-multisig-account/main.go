...
func main() {
	...
	if len(publicKeys) < 5 {
		fmt.Println("Not enough keys provided, five were required for 3-level multisig account.")

	} else if len(publicKeys) >= 5 {
		accounts := getAccounts(publicKeys)

		trx := addCosigner(accounts[0], accounts[1].PublicAccount, accounts[2].PublicAccount)
		trxs = append(trxs, trx)

		trx = addCosigner(accounts[3], accounts[4].PublicAccount, accounts[0].PublicAccount)
		trxs = append(trxs, trx)

		aggBondedTx := createAggregateBondedTx(trxs)

		//first account in .txt file will pay for lock funds and tx fee
		firstMultisigAccInfo := getAccountInfo(accounts[0])
		isSufficient := isXpxBalanceSufficient(aggBondedTx, firstMultisigAccInfo)

		if !isSufficient {
			fmt.Printf("Not enough XPX balance!")
		} else if isSufficient {
			fmt.Println("Enough XPX balance, generating multi-level multisig account...")
			signAggBondedTxWithCosignatures(aggBondedTx, accounts[0], accounts[1:])
		}
	}

}

func addCosigner(multisig *sdk.Account, cosignatory1 *sdk.PublicAccount, cosignatory2 *sdk.PublicAccount) *sdk.ModifyMultisigAccountTransaction {
	...
	convertIntoMultisigTx, err := client.NewModifyMultisigAccountTransaction(
		...
		[]*sdk.MultisigCosignatoryModification{
			{Type: sdk.Add, PublicAccount: cosignatory1},
			{Type: sdk.Add, PublicAccount: cosignatory2},
		},
	)
	if err != nil {
		panic(err)
	...
	convertIntoMultisigTx.ToAggregate(multisig.PublicAccount)
...
func getXpxBalanceByAccount(accInfo *sdk.AccountInfo) float64 {
...
func getTotalFee(aggTx *sdk.AggregateTransaction) float64 {
	aggTxFee := getTotalAggTxFee(aggTx)
	totalTxFee := LOCK_FUND + LOCK_FUND_TX_FEE + aggTxFee
...
// Read private keys from .txt file
